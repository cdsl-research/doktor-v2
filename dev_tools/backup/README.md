...
外部のpip依存はなく、`backup.py` 自体は標準ライブラリのみで動作する（Python 3.10+）。
...
# 出力先を指定
python3 backup.py --output-dir /var/backups/doktor-v2

# MongoDBだけ、MinIOだけなど対象を絞る
python3 backup.py --types mongo
python3 backup.py --types minio,elasticsearch
...
- MongoDB / MinIOの認証情報は実行時に `kubectl get secret` で取得し、平文を保存・ハードコードしない。
